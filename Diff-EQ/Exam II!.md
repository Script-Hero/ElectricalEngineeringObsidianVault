...
1. If $g(t)$ is a polynomial in the form $g(t)=a_{0}t^{n}+ a_{1}t^{n-1} + ... + a_{n}$
	...
	- Where $s$ is the number of times $0$ is a root of the characteristic equation
2. If $g(t)=(a_{0}t^{n}+ a_{1}t^{n-1} + ... + a_{n})e^{rt}$
	Then $y_{p}(t)=t^{s}(A_{0}t^{n}+ A_{1}t^{n-1}+...+A_{n})e^{rt}$
	1. If $r$ is *not* a root of the characteristic equation, then $s=0$
	2. If $r$ is a *simple* root of the characteristic equation, then $s=1$
	3. If $r$ is a *double* root of the characteristic equation, then $s=2$
