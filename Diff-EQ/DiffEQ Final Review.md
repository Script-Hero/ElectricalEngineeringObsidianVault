...
- Determine the lower bound for the radius of convergence about $x_{0} =0$
	- $P(x)=x^{2}+1$ --> $x_{0}=0$ is not a root so it is an *ordinary point* meaning we can have a power series solution
	- Radius of convergence LOWER BOUND is distance between $x_{0}$ and the nearest root of $P(x)$ to $x_{0}$
